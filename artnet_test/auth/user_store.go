...
func init() {
	...
	if user, _ := globalUserStore.FindByUsername("admin"); user  == nil {
		log.Println("NO ADMIN, CREATING...")
		user,_ := NewUser("admin", "root@localhost", "12345678")
		globalUserStore.Save(user)
	}
}
